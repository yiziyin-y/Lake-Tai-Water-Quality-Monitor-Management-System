...
import com.glyk.heos.service.水生细密硅藻门Service;
...
@Service
public class 水生细密硅藻门ServiceImpl implements 水生细密硅藻门Service {
    @Override
    public Result get水生细密硅藻门Report(String place, int col) {
        Result result=new Result();
        Map resultMap=new HashMap();
        水生细密硅藻门Example s水生细密硅藻门Example=new 水生细密硅藻门Example();
        水生细密硅藻门Example.Criteria criteria=s水生细密硅藻门Example.createCriteria().and采样点位EqualTo(place);
        List<水生细密硅藻门> 水生细密硅藻门List=s水生细密硅藻门Mapper.selectByExample(s水生细密硅藻门Example);
        List<String> timelist=new ArrayList<String>();
        List<Double> reslist=new ArrayList<Double>();
        String[] fieldList=EntityUtil.getClassColmun("com.glyk.heos.dal.dao.水生细密硅藻门");
        String value="";
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM");
        for (水生细密硅藻门 data:水生细密硅藻门List)
        {
    ...
    public Result get水生细密硅藻门Info(Date startDate, Date endDate, int pageIndex, int pageSize) {
        ...
        水生细密硅藻门Example s水生细密硅藻门Example = new 水生细密硅藻门Example();
        水生细密硅藻门Example.Criteria criteria = s水生细密硅藻门Example.createCriteria();
        PageHelper.startPage(pageIndex,pageSize);
        List<水生细密硅藻门> 水生细密硅藻门List = s水生细密硅藻门Mapper.selectByExample(s水生细密硅藻门Example);

        PageInfo pageInfo = new PageInfo(水生细密硅藻门List);

        resultMap.put("tHead",fieldList);
        resultMap.put("tBody",水生细密硅藻门List);
    ...
    public boolean deleteRow(int rowId) {
        ...
        int result =   s水生细密硅藻门Mapper.deleteByExample(s水生细密硅藻门Example);
    ...
    public boolean updateRow(水生细密硅藻门 record) {
        ...
        int result=s水生细密硅藻门Mapper.updateByExampleSelective(record,s水生细密硅藻门Example);
        return result==1?true:false;
    }

    public boolean insert(水生细密硅藻门 record) {
        int result=s水生细密硅藻门Mapper.insert(record);
        return result==1?true:false;
    }
    public Result get水生细密硅藻门统计Info(int col, Date startDate, Date endDate) {
        ...
        Map resultMap=new HashMap();
        水生细密硅藻门Example s水生细密硅藻门Example=new 水生细密硅藻门Example();
        水生细密硅藻门Example.Criteria criteria=s水生细密硅藻门Example.createCriteria().and日期Between(startDate,endDate);
//        PageHelper.startPage(pageIndex,pageSize);
        List<水生细密硅藻门> 水生细密硅藻门List=s水生细密硅藻门Mapper.selectByExample(s水生细密硅藻门Example);
        Collections.sort(水生细密硅藻门List, new Comparator<水生细密硅藻门>() {
            @Override
            public int compare(水生细密硅藻门 o1, 水生细密硅藻门 o2) {
                return o1.get采样点位().compareTo(o2.get采样点位());
            }
        });
        String[] fieldList={"采样点位","平均值","最大值","最小值","标准差"};
        String[] fieldList2=EntityUtil.getClassColmun("com.glyk.heos.dal.dao.水生细密硅藻门");
    ...
    public Result get水生细密硅藻门统计Info2(String place, Date startDate, Date endDate) {
        ...
        Map resultMap=new HashMap();
        水生细密硅藻门Example s水生细密硅藻门Example=new 水生细密硅藻门Example();
        水生细密硅藻门Example.Criteria criteria=s水生细密硅藻门Example.createCriteria().and日期Between(startDate,endDate).and采样点位EqualTo(place);
//        PageHelper.startPage(pageIndex,pageSize);
        List<水生细密硅藻门> 水生细密硅藻门List=s水生细密硅藻门Mapper.selectByExample(s水生细密硅藻门Example);
        List<水生细密硅藻门> 水生细密硅藻门ListRes=new ArrayList<水生细密硅藻门>();
        水生细密硅藻门 d1=new 水生细密硅藻门();
        水生细密硅藻门 d2=new 水生细密硅藻门();
        水生细密硅藻门 d3=new 水生细密硅藻门();
        水生细密硅藻门 d4=new 水生细密硅藻门();
        String[] fieldList=EntityUtil.getClassColmun("com.glyk.heos.dal.dao.水生细密硅藻门");
        List<Double> list=new ArrayList<Double>();
        StaDao sta=null;
        String value="";
...
//                System.out.println(sta.getAverage()+" "+sta.getMax()+" "+sta.getMax()+" "+sta.getMin());
        ...
        d3.set采样点位("最小值");
        d4.set采样点位("标准差");
        水生细密硅藻门ListRes.add(d1);
        水生细密硅藻门ListRes.add(d2);
        水生细密硅藻门ListRes.add(d3);
        水生细密硅藻门ListRes.add(d4);
        PageInfo pageInfo = new PageInfo(水生细密硅藻门ListRes);
        fieldList[2]="";
        resultMap.put("tHead",fieldList);
        resultMap.put("tBody",水生细密硅藻门ListRes);
        result.setTotal((int)pageInfo.getTotal());
        result.setRows(resultMap);
        return result;
    }
