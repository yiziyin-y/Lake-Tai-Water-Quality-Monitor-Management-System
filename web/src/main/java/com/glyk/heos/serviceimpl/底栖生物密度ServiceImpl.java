...
import com.glyk.heos.service.底栖生物密度Service;
...
@Service
public class 底栖生物密度ServiceImpl implements 底栖生物密度Service {

    @Resource
    底栖生物密度Mapper s底栖生物密度Mapper;
    @Override
    public Result get底栖生物密度Info(Date startDate, Date endDate, int pageIndex, int pageSize) {
        Result result = new Result();
        Map resultMap =  new HashMap();
        底栖生物密度Example s底栖生物密度Example = new 底栖生物密度Example();
        底栖生物密度Example.Criteria criteria = s底栖生物密度Example.createCriteria().and日期Between(startDate,endDate);
        PageHelper.startPage(pageIndex,pageSize);
        List<底栖生物密度> 底栖生物密度List = s底栖生物密度Mapper.selectByExample(s底栖生物密度Example);
        String[] fieldList = EntityUtil.getClassColmun("com.glyk.heos.dal.dao.底栖生物密度");
        PageInfo pageInfo = new PageInfo(底栖生物密度List);
        resultMap.put("tHead",fieldList);
        resultMap.put("tBody",底栖生物密度List);
    ...
    public boolean deleteRow(int rowId) {
        ...
        底栖生物密度Example s底栖生物密度Example = new 底栖生物密度Example();
        底栖生物密度Example.Criteria criteria = s底栖生物密度Example.createCriteria();
        criteria.andIdEqualTo(rowId);
        int result =   s底栖生物密度Mapper.deleteByExample(s底栖生物密度Example);
    ...
    public boolean updateRow(底栖生物密度 record) {
        ...
        int result=s底栖生物密度Mapper.updateByExampleSelective(record,s底栖生物密度Example);
        return result==1?true:false;
    }

    public boolean insert(底栖生物密度 record) {
        int result=s底栖生物密度Mapper.insert(record);
        return result==1?true:false;
    }
...
//        PageHelper.startPage(pageIndex,pageSize);
        List<底栖生物密度> 底栖生物密度List=s底栖生物密度Mapper.selectByExample(s底栖生物密度Example);
        Collections.sort(底栖生物密度List, new Comparator<底栖生物密度>() {
            @Override
            public int compare(底栖生物密度 o1, 底栖生物密度 o2) {
                return o1.get采样点位().compareTo(o2.get采样点位());
            }
        });
        String[] fieldList={"采样点位","平均值","最大值","最小值","标准差"};
        String[] fieldList2=EntityUtil.getClassColmun("com.glyk.heos.dal.dao.底栖生物密度");
    ...
    public Result get底栖生物密度统计Info2(String place, Date startDate, Date endDate) {
        ...
        Map resultMap=new HashMap();
        底栖生物密度Example s底栖生物密度Example=new 底栖生物密度Example();
        底栖生物密度Example.Criteria criteria=s底栖生物密度Example.createCriteria().and日期Between(startDate,endDate).and采样点位EqualTo(place);
//        PageHelper.startPage(pageIndex,pageSize);
        List<底栖生物密度> 底栖生物密度List=s底栖生物密度Mapper.selectByExample(s底栖生物密度Example);
        List<底栖生物密度> 底栖生物密度ListRes=new ArrayList<底栖生物密度>();
        底栖生物密度 d1=new 底栖生物密度();
        底栖生物密度 d2=new 底栖生物密度();
        底栖生物密度 d3=new 底栖生物密度();
        底栖生物密度 d4=new 底栖生物密度();
        String[] fieldList=EntityUtil.getClassColmun("com.glyk.heos.dal.dao.底栖生物密度");
        List<Double> list=new ArrayList<Double>();
        StaDao sta=null;
        String value="";
...
//                System.out.println(sta.getAverage()+" "+sta.getMax()+" "+sta.getMax()+" "+sta.getMin());
        ...
        d3.set采样点位("最小值");
        d4.set采样点位("标准差");
        底栖生物密度ListRes.add(d1);
        底栖生物密度ListRes.add(d2);
        底栖生物密度ListRes.add(d3);
        底栖生物密度ListRes.add(d4);
        PageInfo pageInfo = new PageInfo(底栖生物密度ListRes);
        fieldList[2]="";
        resultMap.put("tHead",fieldList);
        resultMap.put("tBody",底栖生物密度ListRes);
        result.setTotal((int)pageInfo.getTotal());
        result.setRows(resultMap);
        return result;
    }
    ...
    public Result get底栖生物密度Report(String place, int col) {
        ...
        Map resultMap=new HashMap();
        底栖生物密度Example s底栖生物密度Example=new 底栖生物密度Example();
        底栖生物密度Example.Criteria criteria=s底栖生物密度Example.createCriteria().and采样点位EqualTo(place);
        List<底栖生物密度> 底栖生物密度List=s底栖生物密度Mapper.selectByExample(s底栖生物密度Example);
